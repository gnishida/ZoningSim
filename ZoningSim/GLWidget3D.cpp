...
GLWidget3D::GLWidget3D(MainWindow* mainWin) : QGLWidget(QGLFormat(QGL::SampleBuffers), (QWidget*)mainWin) {
	...
	camera.dz = 8000;
	zoning = new Zoning(8000, 80);

	// 重みを適当にセットする
	QMap<QString, float> weights;
	weights["accessibility_highway"] = 0.02f;
	weights["accessibility_avenue"] = 0.01f;
	weights["accessibility_street"] = 0.002f;
	weights["pollution"] = 0.002f;
	zoning->setWeights(weights);
}
